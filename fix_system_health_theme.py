...
hero_old = '''<div
    ...
    style="background:linear-gradient(135deg,#1e40af 0%,#1e3a8a 45%,#0f172a 100%)">
    <div
      class="absolute inset-0 opacity-10"
      style="background-image:radial-gradient(circle,#fff 1px,transparent 1px);background-size:28px 28px">
    </div>
    <div
      class="absolute -top-16 -right-16 w-80 h-80 rounded-full opacity-15"
    ...
    <div class="relative px-7 py-7 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
      <div>
        <div class="flex flex-wrap items-center gap-2 mb-3">
          <span
            class="px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[3px] text-white/80"
        ...
        <h1 class="text-3xl font-black text-white uppercase tracking-tight leading-tight">
          {{ page.title }}<span class="text-blue-300">.</span>
        </h1>
      ...
      <div class="w-16 h-16 rounded-2xl bg-white/10 flex items-center justify-center ring-1 ring-white/15">
        <lucide-icon [name]="page.icon" class="w-8 h-8 text-white"></lucide-icon>
      </div>
    </div>
  </div>'''
...
with open('frontend/src/app/dashboard/system-health/system-health.component.html', 'w') as f:
    f.write(content)
print("Updated system-health.component.html")
