...
import {
  LucideAngularModule,
  LayoutDashboard, User, Image, Layers, Briefcase, Cpu, Users, Files,
  HelpCircle, Folder, CreditCard, BookOpen, Plus, Palette, Building2,
  Key, WalletCards, ShieldCheck, Network, BarChart3, Globe, RefreshCcw,
  Mail, Settings, MenuSquare, MessageSquare, LogOut, Search, X,
  CheckCircle2, XCircle, Clock, Database, ChevronDown, Eye, Pencil,
  TrendingUp, UserCheck, UserX, UserCog, Activity, Zap, ArrowUpRight,
  ArrowDownRight, Sparkles, CircleDot, Bell, Package, Server, Lock,
  BarChart2, PieChart, LineChart, Calendar, Star, Rocket, Shield, Wifi,
  WifiOff, AlertTriangle, Info, MapPin, Hash, Briefcase as BriefcaseIcon
} from 'lucide-angular';
...
interface DashboardCategory {
  id: string; label: string; icon: any; list: string; add: string;
  roles: BackendRole[]; description: string; color: string;
}

interface SuperadminClient {
  id: string; full_name: string; email: string; country_code?: string; phone?: string;
  company_name: string; business_type: string; business_size?: string; country: string;
  city: string; state?: string; address?: string; status: string; created_at: string;
  updated_at?: string; tenant_id: string; auth_role: string; auth_status: string;
  plan_name: string; subscription_status: string; billing_cycle?: string; amount?: number; license_status?: string; enabled_modules?: number;
}

interface TenantModule {
  id: string; name: string; category: string; price_monthly: number; price_yearly: number;
  route: string; icon: string; enabled: boolean; source: string;
}

interface CatalogPlan {
  id: string; name: string; monthly_price: number; yearly_price: number;
  status: string; modules: { id: string; name: string }[];
}

interface TenantSubscriptionSummary {
  status: string; plan_id: string; billing_cycle: string;
  plan_base_amount?: number; module_addons_amount?: number; amount: number;
}

interface SuperadminClientsResponse {
  ok: boolean; clients: SuperadminClient[]; count: number; total: number;
  totals: { pending: number; approved: number; rejected: number; all: number; };
  limit: number; offset: number; has_more: boolean; message?: string;
}

@Component({
  selector: 'app-dashboard',
  standalone: true,
  imports: [CommonModule, LucideAngularModule, DashboardOverviewComponent, DashboardAllClientsComponent, DashboardPlansPricingComponent, DashboardModuleCatalogComponent, DashboardSystemHealthComponent, DashboardSaasControlComponent],
  templateUrl: './dashboard.component.html'
})
export class DashboardComponent implements OnInit, OnDestroy {
  // SCS_DASHBOARD_SAAS_CONTROL_BRIDGE_V1
  readonly saasControlBusinessLabel =
    (): string =>
      this.saasClientBusinessLabel();

  readonly saasControlEnabledCount =
    (): number =>
      this.scsSuperEnabledCount();

  readonly saasControlAssignablePlans =
    (): any[] =>
      this.saasAssignablePlans();

  readonly saasControlBusinessPlanPrice =
    (plan: any): number =>
      this.saasBusinessPlanPrice(plan);

  readonly saasControlBusinessPlanModuleCount =
    (plan: any): number =>
      this.saasBusinessPlanModuleCount(plan);

  readonly saasControlBusinessPlanModuleNames =
    (plan: any): string =>
      this.saasBusinessPlanModuleNames(plan);

  readonly saasControlSelectedPackageTotal =
    (): number =>
      this.saasSelectedPackageTotal();

  readonly saasControlSuperCategories =
    (): string[] =>
      this.scsSuperCategories();

  readonly saasControlModuleCategoryLabel =
    (category: any): string =>
      this.moduleCategoryLabel(String(category || ''));

  readonly saasControlSuperVisibleModules =
    (): any[] =>
      this.scsSuperVisibleModules();

  readonly saasControlSuperTrackModule =
    (index: number, module: any): string =>
      this.scsSuperTrackModule(index, module);

  readonly saasControlSuperModuleSaving =
    (module: any): boolean =>
      this.scsSuperModuleSaving(module);

  readonly saasControlSuperIsModuleEnabled =
    (module: any): boolean =>
      this.scsSuperIsModuleEnabled(module);

  readonly saasControlSuperModulePrice =
    (module: any): number =>
      this.scsSuperModulePrice(module);

  saasControlSelectClient(
    clientId: any
  ): void {
    void this.selectSaasControlClient(
      String(clientId || '')
    );
  }

  saasControlSetBillingCycle(
    value: any
  ): void {
    this.saasClientSetBillingCycle(
      String(value || '')
    );
  }

  saasControlSelectPlan(
    planId: any
  ): void {
    this.saasClientSelectPlan(
      String(planId || '')
    );
  }
  ...
  saasControlSetModuleSearch(
    value: any
  ): void {
    this.scsSuperSetModuleSearch(
      String(value || '')
    );
  }

  saasControlSetModuleCategory(
    value: any
  ): void {
    this.scsSuperSetModuleCategory(
      String(value || 'all')
    );
  }

  saasControlSetModuleEnabled(
    change: { module: any; enabled: boolean }
  ): void {
    void this.scsSuperSetModuleEnabled(
      change.module,
      change.enabled
    );
  }
  // SCS_DASHBOARD_MODULE_CATALOG_BRIDGE_V1
  readonly moduleCatalogCountByCategory =
    (category: any): number =>
      this.countModulesByCategory(category);

  readonly moduleCatalogFormatCategory =
    (category: any): string =>
      this.formatModuleCategory(category);

  readonly moduleCatalogEditorTitleFn =
    (): string =>
      this.moduleCatalogEditorTitle();

  moduleCatalogSetCategory(
    category: any
  ): void {
    this.moduleCatalogCategory = String(
      category || 'all'
    );
  }

  moduleCatalogSetSearch(
    value: any
  ): void {
    this.moduleCatalogSearch = String(
      value || ''
    );
  }

  moduleCatalogSetDraftField(
    change: { field: string; value: any }
  ): void {
    this.setModuleCatalogDraftField(
      change.field,
      change.value
    );
  }
  // SCS_DASHBOARD_PLANS_PRICING_BRIDGE_V1
  readonly plansPricingIsCustom =
    (): boolean =>
      this.businessPlanIsCustom();

  readonly plansPricingModuleScope =
    (module: any): string =>
      this.businessPlanModuleScope(module);

  readonly plansPricingFormatModuleCategory =
    (category: any): string =>
      this.formatModuleCategory(category);

  readonly plansPricingIsModuleSelected =
    (module: any): boolean =>
      this.isPlanBuilderModuleSelected(module);

  readonly plansPricingModuleCount =
    (plan: any): number =>
      this.planBuilderModuleCount(plan);

  plansPricingSetDraftField(
    change: {
      field: 'monthly_price' |
        'yearly_price' |
        'status' |
    ...
    this.setPlanBuilderDraftField(
      change.field,
      change.value
    );
  }

  plansPricingSetSearch(
    value: string
  ): void {
    this.planBuilderSearch = String(
      value || ''
    );
  }
  // SCS_DASHBOARD_ALL_CLIENTS_BRIDGE_V1
  readonly allClientsGetInitials =
    (client: any): string =>
      this.getInitials(client);

  allClientsSelectPage(page: number): void {
    this.clientPage = Math.max(
      1,
      Number(page || 1)
    );
  ...
  readonly overviewGetGreeting = (): string =>
    this.getGreeting();

  readonly overviewGetInitials =
    (client: any): string =>
      this.getInitials(client);
  ...
  scsSuperTab: 'overview' | 'plan' | 'modules' | 'billing' | 'access' | 'documents' | 'audit' = 'overview';
  ...
  scsSuperOpen(client: SuperadminClient): void {
    ...
    this.scsSuperAudit = [{ time: new Date().toLocaleString(), action: 'Opened profile', detail: client.email || client.full_name || 'client' }];
  ...
  scsSuperSetTab(tab: 'overview' | 'plan' | 'modules' | 'billing' | 'access' | 'documents' | 'audit'): void {
  ...
  scsSuperInitial(): string {
    ...
    return String(c.full_name || c.company_name || c.email || 'S').slice(0,1).toUpperCase();
  }

  scsSuperAuditAdd(action: string, detail: string): void {
    this.scsSuperAudit = [{ time: new Date().toLocaleString(), action, detail }, ...this.scsSuperAudit].slice(0, 30);
  }
  ...
  scsSuperPlanBase(): number {
    const selectedID = String(
      ...
      this.selectedPlanId ||
      this.tenantSubscription.plan_id ||
      ''
    );

    const matrixPlan = this.saasClientPlans.find(
      (plan: any) =>
        String(plan?.id || plan?.plan_id || '') === selectedID
    );
    ...
    const serverBase = Number(
      this.tenantSubscription.plan_base_amount || 0
    );

    if (
      serverBase > 0 &&
      String(this.tenantSubscription.plan_id || '') === selectedID
    ) {
      return serverBase;
    }
  ...
  scsSuperModulePrice(module: TenantModule): number {
    return this.selectedBillingCycle === 'yearly' ? Number(module.price_yearly || 0) : Number(module.price_monthly || 0);
  }
  ...
  scsSuperAddons(): number {
    ...
    return this.tenantModules
      ...
      .reduce(
        (sum, module) =>
          sum + this.scsSuperModulePrice(module),
        0
      );
  }
  ...
  scsSuperTotal(): number {
    ...
    return live || Number(this.tenantSubscription.amount || 0) || Number(this.scsSuperProfileClient?.amount || 0) || 0;
  }
  ...
  scsSuperApplyServerSubscription(raw: any): void {
    ...
    this.selectedBillingCycle = this.tenantSubscription.billing_cycle === 'yearly' ? 'yearly' : 'monthly';
    this.scsSuperApplyLive();
  }

  scsSuperCategories(): string[] {
    const cats = Array.from(new Set(this.tenantModules.map((m: any) => String(m.category || 'business'))));
  ...
  async scsSuperLoadModules(): Promise<void> {
    ...
    try {
      ...
      const res = await fetch('/api/superadmin/tenant-modules?' + params.toString() + '&_live=' + Date.now(), {
        method: 'GET',
        credentials: 'include',
        cache: 'no-store'
      });
      ...
      this.showActionPopup('Modules Load Failed', error instanceof Error ? error.message : 'Modules load nahi huwe.', false);
    } finally {
  ...
  async scsSuperApplyPlan(): Promise<void> {
    ...
    try {
      ...
      this.scsSuperApplyServerSubscription(data.subscription);
      this.scsSuperAuditAdd('Plan assigned', this.selectedPlanId + ' / ' + this.selectedBillingCycle);
      ...
      this.scsSuperApplyLive();
      this.showActionPopup('Plan Save Failed', error instanceof Error ? error.message : 'Plan save nahi hua.', false);
    } finally {
  ...
  async scsSuperSetModuleEnabled(module: TenantModule, enabled: boolean): Promise<void> {
    ...
    if (_el) requestAnimationFrame(() => { _el.scrollTop = _y; });
    ...
    try {
      ...
      this.scsSuperAuditAdd(next ? 'Module granted' : 'Module revoked', (module as any).name || moduleId);
      this.showActionPopup(next ? 'Access Granted ✓' : 'Access Removed ✓', 'Saved live to tenant_modules.', true);
    } catch (error) {
      this.tenantModules = beforeModules;
      this.tenantSubscription = beforeSub;
      this.scsSuperModuleEnabledState = beforeState;
      this.showActionPopup('Module Save Failed', error instanceof Error ? error.message : 'Module save nahi hua.', false);
    } finally {
  ...
  async scsSuperSaveModulePrice(module: TenantModule): Promise<void> {
    ...
    try {
      ...
      this.tenantSubscription = beforeSub;
      this.scsSuperApplyLive();
      this.showActionPopup('Price Save Failed', error instanceof Error ? error.message : 'Price save nahi hui.', false);
    } finally {
      const ids = new Set(this.scsSuperSavingModuleIds);
  ...
  scsSuperInvoiceDraft(): void {
    ...
    this.showActionPopup('Invoice Draft Ready', 'Invoice backend next phase mein attach hoga.', true);
  }

  scsSuperResetPassword(): void {
    this.scsSuperAuditAdd('Password reset requested', this.scsSuperProfileClient?.email || 'client');
  ...
  scsNormalizeCatalogPlans(plans: any[]): any[] {
    ...
    return (Array.isArray(plans) ? plans : [])
      .filter((p: any) => allowed.has(String(p?.id || '')) && String(p?.status || 'active') === 'active')
  ...
  planBuilderModuleIDs(plan: any): string[] {
    ...
    return raw
      .map((x: any) => String((typeof x === 'string' ? x : (x?.id || x?.module_id || '')) || '').trim())
      .filter(Boolean);
  }
  ...
  isPlanBuilderModuleSelected(module: any): boolean {
    ...
    const ids = Array.isArray(this.planBuilderDraft?.module_ids) ? this.planBuilderDraft.module_ids : [];
  ...
  togglePlanBuilderModule(module: any): void {
    const id = String(module?.id || '');
    if (!id) return;
    const current = Array.isArray(this.planBuilderDraft?.module_ids) ? [...this.planBuilderDraft.module_ids] : [];
  ...
  async savePlanBuilder(): Promise<void> {
    ...
    try {
      ...
      if (result.catalog) {
        this.moduleCatalog = Array.isArray(result.catalog?.modules) ? result.catalog.modules : this.moduleCatalog;
        this.catalogPlans = this.scsNormalizeCatalogPlans(Array.isArray(result.catalog?.plans) ? result.catalog.plans : this.catalogPlans);
      } else {
        await this.loadModuleCatalogForSuperadmin();
      }
  ...
  private applyBusinessPlanMatrixResponse(result: any): void {
    ...
    this.businessPlanSelectedType = String(result?.selected_business?.id || this.businessPlanSelectedType || '');
    this.businessPlanPlans = Array.isArray(result?.plans) ? result.plans : [];

    const currentID = this.planBuilderSelectedPlanId;
    const selected = this.businessPlanPlans.find((p: any) => String(p?.id || p?.plan_id) === currentID)
  ...
  async loadBusinessPlanMatrix(businessType = ''): Promise<void> {
    ...
    try {
      ...
      const response = await fetch('/api/superadmin/business-plan-matrix?_live=' + Date.now() + query, {
        method: 'GET',
        credentials: 'include',
        cache: 'no-store',
  ...
  get filteredBusinessPlanModules(): any[] {
    const query = String(this.planBuilderSearch || "").trim().toLowerCase();
    const businessCategory = String(this.businessPlanSelectedInfo?.module_category || '');

    return (this.moduleCatalog || [])
      .filter((module: any) => {
        if (!query) return true;
        const text = [module?.id, module?.name, module?.category]
          .join(' ')
          .toLowerCase();
  ...
  async saveBusinessPlanMatrix(): Promise<void> {
    ...
    try {
      const response = await fetch('/api/superadmin/business-plan-matrix/save?_live=' + Date.now(), {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      ...
      this.applyBusinessPlanMatrixResponse(result);
      this.businessPlanMessage = 'Business plan saved. Included modules: ' + payload.module_ids.length;
      this.businessPlanOk = true;
      this.showActionPopup(
        'Business Plan Saved ✓',
        String(this.businessPlanSelectedInfo?.name || this.businessPlanSelectedType) + ' · ' + payload.plan_id,
        true
      );
    } catch (error: any) {
  ...
  openModuleCatalogCreate(): void {
    const cat = this.moduleCatalogCategory && this.moduleCatalogCategory !== 'all' ? this.moduleCatalogCategory : '';
  ...
  async saveModuleCatalogEditor(): Promise<void> {
    ...
    try {
      const endpoint = this.moduleCatalogEditorMode === 'edit'
      ...
      this.moduleCatalogEditorOk = true;
      this.showActionPopup(this.moduleCatalogEditorMode === 'edit' ? 'Module Updated ✓' : 'Module Created ✓', draft.name, true);
      this.moduleCatalogEditorOpen = false;
    } catch (error) {
  ...
  tenantSubscription: TenantSubscriptionSummary = { status: 'none', plan_id: '', billing_cycle: '', plan_base_amount: 0, module_addons_amount: 0, amount: 0 };
  selectedPlanId = '';
  ...
  customTheme = {
    sidebar: '#1e40af', header: '#ffffff', footer: '#ffffff', background: '#f1f5f9'
  };
  ...
  readonly superadminSaasPages: any = {
    'saas-control': {
      ...
      groups: [
        { title: 'Subscription Control', items: ['Plans & Pricing', 'Monthly / Yearly Billing', 'Trial Management', 'Renewals', 'Suspension / Reactivation'] },
        { title: 'Tenant Control', items: ['Tenant Management', 'Module Entitlements', 'ClientAdmin Access', 'Feature Flags', 'Usage Limits'] },
        { title: 'Audit & Safety', items: ['Billing Audit', 'Module Audit', 'Admin Actions', 'Data Export', 'Restore Points'] }
      ]
    },
    'module-catalog': {
      icon: Package,
      ...
      subtitle: 'Master catalog for all SaaS, ERP, POS, CRM, clinic, school and restaurant modules.',
      quickLinks: [
        { title: 'Business Suites', view: 'business-suites' },
      ...
      groups: [
        { title: 'Core / Admin', items: ['Dashboard', 'Company Profile', 'Branches', 'Users', 'Roles', 'Files', 'Calendar', 'Notifications'] },
        { title: 'Sales / CRM', items: ['Leads', 'Customers', 'Deals', 'Follow-ups', 'Quotations', 'Contracts', 'Loyalty', 'Feedback'] },
        { title: 'POS / Retail', items: ['POS', 'Barcode', 'Products', 'Inventory', 'Purchase Orders', 'Suppliers', 'Returns', 'Ecommerce'] },
        { title: 'Restaurant', items: ['Menu', 'Tables', 'Floor Plan', 'Kitchen Display', 'Waiter App', 'Reservations', 'Delivery', 'Ingredients'] },
        { title: 'Finance', items: ['Invoices', 'Payments', 'Receipts', 'Expenses', 'Ledger', 'Tax', 'Bank Accounts', 'Subscriptions'] },
        { title: 'Education', items: ['Students', 'Admissions', 'Classes', 'Timetable', 'Attendance', 'Fees', 'Exams', 'Parent Portal'] },
        { title: 'Clinic / Dental', items: ['Appointments', 'Patients', 'EHR', 'Prescriptions', 'Dental Charting', 'Lab Reports', 'Queue', 'Patient Portal'] },
        { title: 'AI / Reports / Integrations', items: ['Reports', 'Sales Analytics', 'AI Forecast', 'Risk Score', 'WhatsApp', 'Stripe', 'Bank API', 'Webhooks'] }
      ]
    },
    'business-suites': {
      icon: BriefcaseIcon,
      ...
      groups: [
        { title: 'Store / Retail Suite', items: ['POS', 'Inventory', 'Barcode', 'Products', 'Suppliers', 'Returns', 'Ecommerce', 'Payments', 'Reports'] },
        { title: 'Restaurant / Cafe Suite', items: ['POS', 'Menu', 'Tables', 'Kitchen Display', 'Waiter App', 'Delivery', 'Reservations', 'Food Inventory'] },
        { title: 'School / Academy Suite', items: ['Students', 'Admissions', 'Classes', 'Attendance', 'Fees', 'Exams', 'Grades', 'Parent Portal'] },
        { title: 'Dental Clinic Suite', items: ['Appointments', 'Patient Records', 'Dental Charting', 'Treatment Plans', 'Prescriptions', 'Lab Reports', 'Billing'] },
        { title: 'Medical Clinic Suite', items: ['Appointments', 'EHR', 'Doctor Schedule', 'Prescriptions', 'Lab Reports', 'Pharmacy Stock', 'Queue'] },
        { title: 'Salon / Spa Suite', items: ['Booking', 'Services Menu', 'Staff Scheduling', 'Packages', 'Memberships', 'Products', 'Commission'] },
        { title: 'Service / Repair Suite', items: ['Tickets', 'Job Cards', 'Work Orders', 'Projects', 'Warranty', 'Field Staff', 'Invoices'] },
        { title: 'Hotel / Rental Suite', items: ['Reservations', 'Check-in / Check-out', 'Room Status', 'Housekeeping', 'Rental Items', 'Deposits'] }
      ]
    },
    'plans-pricing': {
      icon: CreditCard,
      ...
      groups: [
        ...
        { title: 'Plan Rules', items: ['Base Monthly Price', 'Base Yearly Price', 'Included Modules', 'Max Users', 'Max Branches', 'Trial Days'] },
        { title: 'Client Overrides', items: ['Custom Amount', 'Module Addons', 'Discounts', 'Manual Activation', 'Renewal Rules'] }
      ]
    },
    'system-health': {
      icon: Server,
      ...
      groups: [
        { title: 'Runtime', items: ['Frontend Build', 'Backend API', 'PostgreSQL', 'SQLite Sync', 'Preview Server'] },
        { title: 'Guardian', items: ['Audit Engine', 'Auto Refresh', 'Health Checks', 'Sync Worker', 'Error Logs'] },
        { title: 'Maintenance', items: ['Backup', 'Restore', 'Version Update', 'Cache Clear', 'Diagnostics'] }
      ]
    },
  };
  ...
  private readonly allCategories: DashboardCategory[] = [
    { id: 'rbac', label: 'RBAC & Roles', icon: ShieldCheck, list: 'Manage Roles', add: 'Permissions', roles: ['super_admin'], description: 'Role-based access control', color: 'violet' },
    { id: 'api-gate', label: 'API Gateway', icon: Network, list: 'Endpoints', add: 'Security Layer', roles: ['super_admin'], description: 'Endpoint management & security', color: 'cyan' },
    { id: 'analytics', label: 'Analytics', icon: BarChart3, list: 'Usage Stats', add: 'Audit Logs', roles: ['super_admin', 'client_admin'], description: 'Reports & audit trails', color: 'orange' },
    { id: 'frontend', label: 'Frontend Site', icon: Globe, list: 'Marketplace', add: 'Site Pages', roles: ['super_admin'], description: 'Marketplace & pages', color: 'teal' },
    { id: 'updater', label: 'Auto Update', icon: RefreshCcw, list: 'Version Control', add: 'Plugins', roles: ['super_admin'], description: 'Version & plugin control', color: 'lime' },
    { id: 'tenants', label: 'Tenant System', icon: Building2, list: 'Multi Company', add: 'Company Create', roles: ['super_admin'], description: 'Multi-tenant architecture', color: 'blue' },
    { id: 'license', label: 'License System', icon: Key, list: 'Software Key', add: 'Device Binding', roles: ['super_admin'], description: 'Key & device management', color: 'amber' },
    { id: 'payments-sys', label: 'Payments', icon: WalletCards, list: 'Billing', add: 'Plans', roles: ['super_admin', 'client_admin'], description: 'Billing & subscription plans', color: 'green' },
    { id: 'admins', label: 'Admins', icon: User, list: 'List Admins', add: 'Add New', roles: ['super_admin', 'client_admin'], description: 'Admin user management', color: 'indigo' },
    { id: 'services', label: 'Services', icon: Layers, list: 'List Services', add: 'Add New', roles: ['super_admin', 'client_admin', 'client_user'], description: 'Service catalogue', color: 'purple' },
    { id: 'teams', label: 'Teams', icon: Users, list: 'List Teams', add: 'Add New', roles: ['super_admin', 'client_admin'], description: 'Team & member management', color: 'pink' },
    { id: 'blogs', label: 'Blogs', icon: BookOpen, list: 'List Blogs', add: 'Add New', roles: ['super_admin'], description: 'Content publishing', color: 'rose' },
    { id: 'settings', label: 'Settings', icon: Settings, list: 'General', add: 'Security', roles: ['super_admin', 'client_admin'], description: 'System configuration', color: 'slate' }
  ];
  ...
  readonly icons = {
    ...
    ArrowDownRight, Sparkles, CircleDot, Bell, Package, Server, Lock,
    BarChart2, PieChart, LineChart, Calendar, Star, Rocket, Shield, Wifi,
    WifiOff, AlertTriangle, Info, MapPin, Hash
  };

  private onlineHandler = () => { this.isOnline = true; this.cdr.detectChanges(); };
  private offlineHandler = () => { this.isOnline = false; this.cdr.detectChanges(); };
  ...
  async ngOnInit(): Promise<void> {
    ...
    setTimeout(() => { void this.businessPlanEnsureLoaded(); }, 0);
    this.isOnline = navigator.onLine;
    window.addEventListener('online', this.onlineHandler);
    window.addEventListener('offline', this.offlineHandler);
    this.updateDateTime();
    this.clockTimer = setInterval(() => { this.updateDateTime(); this.cdr.detectChanges(); }, 60000);
  ...
  private async silentRefresh(): Promise<void> {
    ...
    try {
      const limit = 100; let offset = 0; let allRows: SuperadminClient[] = []; let lastResult: any = null;
      while (true) {
        const params = new URLSearchParams({ status: 'all', limit: String(limit), offset: String(offset) });
        const response = await fetch(`/api/superadmin/clients?${params.toString()}&_t=${Date.now()}`, { method: 'GET', credentials: 'include', cache: 'no-store' });
      ...
      this.applyClientLocalView();
      const now = new Date();
      this.lastRefreshedAt = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
    } catch {} 
    finally { this.cdr.detectChanges(); } // 🚀 FIX: Silent refresh update
  }
  ...
  private async loadCurrentUser(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok || !result?.ok || !result.user) { await this.router.navigateByUrl('/login'); return; }
      ...
      if (user.role === 'super_admin') { this.activeView = 'all-tenants'; this.clientStatusTab = 'all'; }
      if (this.router.url.startsWith('/superadmin') && user.role !== 'super_admin') { await this.router.navigateByUrl('/dashboard'); return; }
      if (this.router.url.startsWith('/dashboard') && user.role === 'super_admin') { await this.router.navigateByUrl('/superadmin'); return; }
      if (user.role === 'super_admin') { await this.loadSuperadminClients(true); }
      else { await this.loadClientAllowedModules(); }
    } catch { await this.router.navigateByUrl('/login'); }
    finally { 
  ...
  async loadSuperadminClients(reset = true): Promise<void> {
    ...
    try {
      ...
      while (true) {
        ...
        const response = await fetch(`/api/superadmin/clients?${params.toString()}&_t=${Date.now()}`, { method: 'GET', credentials: 'include', cache: 'no-store' });
        const result: any = await response.json().catch(() => null);
        if (!response.ok || !result?.ok) { this.actionMessage = result?.message || 'Could not load clients.'; this.actionSuccess = false; return; }
        lastResult = result;
        const rows = Array.isArray(result.clients) ? result.clients : [];
        allRows = [...allRows, ...rows];
        offset += rows.length;
      ...
      this.lastRefreshedAt = now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true });
      this.autoRefreshCountdown = 30;
    } catch { this.actionMessage = 'Network error. Please retry.'; this.actionSuccess = false; }
    finally { 
  ...
  applyClientLocalView(): void {
    ...
    if (this.clientStatusTab !== 'all') {
      rows = rows.filter(c => {
        if (this.clientStatusTab === 'pending') return c.status === 'pending' || c.status === 'pending_verification';
  ...
  get clientTotalPages(): number { return Math.max(1, Math.ceil((this.clientTotal || 0) / this.clientLimit)); }
  nextClientPage(): void { if (this.clientPage < this.clientTotalPages) { this.clientPage++; this.applyClientLocalView(); } }
  previousClientPage(): void { if (this.clientPage > 1) { this.clientPage--; this.applyClientLocalView(); } }

  changeClientStatusTab(tab: ClientStatusTab | string): void {
    const safeTab: ClientStatusTab = (tab === 'pending' || tab === 'approved' || tab === 'rejected' || tab === 'all') ? tab : 'all';
  ...
  openConfirm(title: string, message: string, action: () => Promise<void>, danger = false): void {
    this.confirmTitle = title; this.confirmMessage = message; this.confirmAction = action;
    this.confirmDanger = danger; this.confirmOpen = true;
    this.cdr.detectChanges(); // 🚀 FIX: Show confirmation modal immediately
  }
  
  closeConfirm(): void { 
    this.confirmOpen = false; this.confirmAction = null; 
    this.cdr.detectChanges(); // 🚀 FIX: Close confirmation modal immediately
  }
  
  async runConfirm(): Promise<void> {
    if (!this.confirmAction) return;
    const fn = this.confirmAction; this.closeConfirm(); await fn();
  }

  moduleIcon(iconName: string): any { return (this.icons as any)[iconName] || this.icons.Package; }

  private moduleToCategory(module: TenantModule): DashboardCategory {
    return {
      id: module.route || module.id, label: module.name, icon: this.moduleIcon(module.icon),
      list: module.name + ' Dashboard', add: 'Settings', roles: ['client_admin', 'client_user', 'support'],
      description: module.category + ' module', color: 'blue'
    };
  }
  ...
  tenantModuleCategories(): string[] {
    const order = ['store', 'school', 'restaurant', 'clinic', 'business', 'finance', 'analytics', 'frontend'];
  ...
  async loadClientAllowedModules(): Promise<void> {
    try {
      const response = await fetch('/api/client/modules?_live=' + Date.now(), { method: 'GET', credentials: 'include', cache: 'no-store' });
      const result: any = await response.json().catch(() => null);
    ...
    finally { this.cdr.detectChanges(); } // 🚀 FIX
  }

  async loadModuleCatalogForSuperadmin(): Promise<void> {
    const response = await fetch('/api/superadmin/modules?_live=' + Date.now(), { method: 'GET', credentials: 'include', cache: 'no-store' });
    const result: any = await response.json().catch(() => null);
    if (response.ok && result?.ok) {
      this.moduleCatalog = Array.isArray(result.catalog?.modules) ? result.catalog.modules : [];
      this.catalogPlans = this.scsNormalizeCatalogPlans(Array.isArray(result.catalog?.plans) ? result.catalog.plans : []);
  ...
  saasAssignablePlans(): any[] {
    ...
    return this.saasClientPlans
      .filter((plan: any) =>
        allowed.has(
          String(plan?.id || plan?.plan_id || '').toLowerCase()
        )
      )
      .sort(
        (a: any, b: any) =>
  ...
  saasBusinessPlanModuleNames(plan: any): string {
    ...
    const names = ids
      .map((id: string) => {
        const module = this.moduleCatalog.find(
  ...
  saasSelectedManualAddonsTotal(): number {
    ...
    const includedModules = new Set(
      Array.isArray(selectedPlan?.module_ids)
        ? selectedPlan.module_ids.map(
            (id: any) => String(id)
          )
        : []
    );

    return this.tenantModules.reduce(
      (total: number, module: any) => {
        ...
        const price =
          this.saasClientBillingCycle === 'yearly'
            ? Number(
                module?.price_yearly ??
                module?.yearly_price_snapshot ??
                0
              )
            : Number(
                module?.price_monthly ??
                module?.monthly_price_snapshot ??
                0
              );
  ...
  saasSelectedPackageTotal(): number {
    return (
      this.saasBusinessPlanPrice(
        this.saasSelectedBusinessPlan()
      ) +
  ...
  async loadSaasClientBusinessPlans(
    ...
    try {
      const businessType = String(
        client.business_type || 'other_business'
      );
      ...
      const response = await fetch(
        '/api/superadmin/business-plan-matrix?' +
          params.toString(),
        {
          method: 'GET',
          credentials: 'include',
      ...
      if (!response.ok || !result?.ok) {
        throw new Error(
          result?.message ||
          'Client business plans load nahi huwe.'
        );
      }

      this.saasClientBusinessInfo =
        result.selected_business || null;

      this.saasClientPlans = Array.isArray(result.plans)
        ? result.plans
        : [];
      ...
      const currentIsAssignable = assignable.some(
        (plan: any) =>
          String(plan?.id || plan?.plan_id || '') === currentPlanID
      );

      const starter = assignable.find(
        (plan: any) =>
          String(plan?.id || plan?.plan_id || '') === 'starter'
      );
      ...
      this.saasClientSelectedPlanId = currentIsAssignable
        ...
        : String(
            ...
            starter?.plan_id ||
            assignable[0]?.id ||
            assignable[0]?.plan_id ||
            ''
          );

      this.saasClientBillingCycle =
        this.tenantSubscription.billing_cycle === 'yearly' ||
        client.billing_cycle === 'yearly'
          ? 'yearly'
          : 'monthly';
  ...
  async assignSaasClientBusinessPlan(): Promise<void> {
    ...
    if (client.status !== 'approved') {
      this.showActionPopup(
        'Approval Required',
        'Plan sirf approved client ko assign hoga.',
        false
      );
      return;
    }
    ...
    if (!this.saasClientSelectedPlanId) {
      this.showActionPopup(
        'Plan Required',
        'Starter, Pro ya Enterprise select karo.',
        false
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        '/api/superadmin/subscriptions/save?_live=' +
          Date.now(),
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok || !result?.ok) {
        throw new Error(
          result?.message || 'Business plan save nahi hua.'
        );
      }

      this.scsSuperApplyServerSubscription(
        result.subscription
      );
      ...
      this.saasClientPlanMessage =
        result.message ||
        'Business plan aur included modules assign ho gaye.';
  ...
  async selectSaasControlClient(clientId: string): Promise<void> {
    ...
    const client = (this.superadminClients || []).find((c: any) => String(c.id) === String(clientId)) || null;
  ...
  startEditClient(): void { 
    if (!this.selectedClient) return; this.editClientDraft = { ...this.selectedClient }; this.isEditMode = true; 
    this.cdr.detectChanges(); // 🚀 FIX
  }
  cancelEditClient(): void { 
    if (!this.selectedClient) return; this.editClientDraft = { ...this.selectedClient }; this.isEditMode = false; 
    this.cdr.detectChanges(); // 🚀 FIX
  }
  ...
  async saveClientEdit(): Promise<void> {
    ...
    try {
      const response = await fetch('/api/superadmin/clients/update', {
        ...
        body: JSON.stringify({ id: this.selectedClient.id, full_name: this.editClientDraft.full_name || '', country_code: this.editClientDraft.country_code || '', phone: this.editClientDraft.phone || '', company_name: this.editClientDraft.company_name || '', business_type: this.editClientDraft.business_type || '', business_size: this.editClientDraft.business_size || '', country: this.editClientDraft.country || '', city: this.editClientDraft.city || '', state: this.editClientDraft.state || '', address: this.editClientDraft.address || '' })
      });
      ...
      if (response.ok && result?.ok) {
        ...
        this.showActionPopup('Client Updated ✓', 'Client information has been saved successfully.', true);
      } else { this.showActionPopup('Update Failed', result?.message || 'Could not save changes.', false); }
    } catch { this.showActionPopup('Network Error', 'Could not reach server.', false); }
    finally { 
  ...
  requestApproveClient(client: SuperadminClient): void {
    this.openConfirm('Approve Client', `Approve ${client.full_name || client.email}? This will activate their account.`, () => this.approveClient(client), false);
  }
  requestRejectClient(client: SuperadminClient): void {
    this.openConfirm('Reject Client', `Reject ${client.full_name || client.email}? Registration will be declined.`, () => this.rejectClient(client), true);
  }
  ...
  async approveClient(client: SuperadminClient): Promise<void> {
    ...
    this.cdr.detectChanges(); // 🚀 FIX
    try {
      const response = await fetch('/api/superadmin/clients/approve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', cache: 'no-store', body: JSON.stringify({ id: client.id }) });
      const result = await response.json().catch(() => null) as { ok?: boolean; message?: string } | null;
      if (response.ok && result?.ok) {
        this.closeClientDetail(); await this.loadSuperadminClients(true);
        this.showActionPopup('Client Approved ✓', `${client.full_name || client.email} approved. Tenant access active.`, true);
      } else { this.showActionPopup('Approve Failed', result?.message || 'Could not approve.', false); }
    } catch { this.showActionPopup('Network Error', 'Could not reach server.', false); }
    finally { 
      this.isClientActionBusy = false; 
      this.cdr.detectChanges(); // 🚀 FIX
  ...
  async rejectClient(client: SuperadminClient): Promise<void> {
    ...
    this.cdr.detectChanges(); // 🚀 FIX
    try {
      const response = await fetch('/api/superadmin/clients/reject', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', cache: 'no-store', body: JSON.stringify({ id: client.id }) });
      const result = await response.json().catch(() => null) as { ok?: boolean; message?: string } | null;
      if (response.ok && result?.ok) {
        this.closeClientDetail(); await this.loadSuperadminClients(true);
        this.showActionPopup('Client Rejected', `${client.full_name || client.email} registration declined.`, false);
      } else { this.showActionPopup('Reject Failed', result?.message || 'Could not reject.', false); }
    } catch { this.showActionPopup('Network Error', 'Could not reach server.', false); }
    finally { 
      this.isClientActionBusy = false; 
      this.cdr.detectChanges(); // 🚀 FIX
  ...
  async logout(): Promise<void> {
    ...
    finally { localStorage.clear(); await this.router.navigateByUrl('/login'); }
  }
  ...
  async navigateTo(view: string): Promise<void> {
    this.activeView = view; this.isThemeMenuOpen = false;
    this.cdr.detectChanges(); // 🚀 FIX
    if (view === 'all-tenants' && this.backendRole === 'super_admin') await this.loadSuperadminClients(true);
  ...
  clientPopupInitial(): string {
    ...
    return String(c.full_name || c.company_name || c.email || 'C').slice(0, 1).toUpperCase();
  }
  ...
  async clientPopupStatusAction(action: 'approve' | 'reject' | 'suspend' | 'restore'): Promise<void> {
    if (!this.clientPopup) return;

    const endpoint =
      action === 'approve' ? '/api/superadmin/clients/approve' :
      action === 'reject' ? '/api/superadmin/clients/reject' :
      action === 'restore' ? '/api/superadmin/clients/approve' :
    ...
    try {
      ...
      const result: any = await response.json().catch(() => null);
      if (!response.ok || !result?.ok) throw new Error(result?.message || action + ' backend route not ready.');
  ...
  async sendClientPasswordResetLink(): Promise<void> {
    ...
    try {
      const response = await fetch('/api/superadmin/clients/password/reset-link?_live=' + Date.now(), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        cache: 'no-store',
      ...
      const result: any = await response.json().catch(() => null);
      if (!response.ok || !result?.ok) throw new Error(result?.message || 'Reset-link backend route next patch me add hoga.');

      this.clientPopupAuditAdd('Password reset link sent', this.clientPopup.email || this.clientPopup.id);
  ...
  async generateClientTemporaryPassword(): Promise<void> {
    ...
    try {
      const response = await fetch('/api/superadmin/clients/password/temp?_live=' + Date.now(), {
        ...
        credentials: 'include',
        cache: 'no-store',
        body: JSON.stringify({ id: this.clientPopup.id, email: this.clientPopup.email, force_change: true })
      });

      const result: any = await response.json().catch(() => null);
      if (!response.ok || !result?.ok) throw new Error(result?.message || 'Temporary-password backend route next patch me add hoga.');

      this.clientPopupTempPassword = String(result.temp_password || result.password || '');
      this.clientPopupAuditAdd('One-time temporary password generated', this.clientPopup.email || this.clientPopup.id);
      this.clientPopupNotice('One-time temporary password generated. Client next login par password change karega.', true);
    } catch (error) {
