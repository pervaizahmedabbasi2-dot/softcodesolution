...
func registerModuleHTTPRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/superadmin/subscriptions/load", handleSuperadminSubscriptionLoad)
...
`, tenantID)
	...
	subscription, subscriptionErr := loadTenantSubscriptionSummary(ctx, db, tenantID)
	if subscriptionErr != nil {
		writeAPIError(w, http.StatusInternalServerError, "subscription_load_failed", "Could not load tenant subscription")
		return
	}
...
`, tenantID, payload.ModuleID, payload.Enabled, payload.Source)
	...
	if recalcErr != nil {
		...
		fallbackSubscription, subscriptionErr := loadTenantSubscriptionSummary(ctx, db, tenantID)
		if subscriptionErr != nil {
			log.Printf("[WARN] subscription_load_failed tenant_id=%s err=%v", tenantID, subscriptionErr)
		} else {
