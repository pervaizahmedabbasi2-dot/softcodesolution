...
import (
	...
	backup "scs-backend/backend/backup"
)

// SCS_PHASE677C_BACKUP_SERVICE_FACTORY
//
// Uses the application's existing environment configuration.
// No credentials are hard-coded.
// No second database architecture is created.

func createSoftCodeBackupService(db *sql.DB) *backup.Service {

	sqlitePath := getenvDefault(
		"SOFTCODE_SQLITE_DB",
		"backend/auth/database/local_softcodesolution.db",
	)
	...
	if postgresDSN == "" {
		...
		dbname := getenvDefault(
			"SCS_DB_NAME",
			"softcodesolution",
		)

		sslmode := getenvDefault(
			"SCS_DB_SSLMODE",
			"disable",
		)
...
func registerHTTPRoutes(
	...
	// SCS_PHASE677C_BACKUP_ROUTES
	backupAPI := backup.NewAPI(backupService)
	backup.RegisterHTTPRoutes(mux, backupAPI)

	// SCS Automatic Failover Control Plane.
	mux.HandleFunc("/api/superadmin/failover/status", handleFailoverStatus)
	mux.HandleFunc("/api/superadmin/failover/evaluate", handleFailoverEvaluate)
	mux.HandleFunc("/api/superadmin/failover/promote", handleFailoverPromote)

	registerAuthHTTPRoutes(mux)
	registerSuperadminHTTPRoutes(mux)
	registerModuleHTTPRoutes(mux)
	registerRbacHTTPRoutes(mux)
	registerAPIGatewayHTTPRoutes(mux)
	registerAPIGatewaySecurityRoutes(mux)
	registerAPIGatewaySecurityPolicyRoutes(mux)
	registerPaymentHTTPRoutes(mux)
...
func openRegisterDB(ctx context.Context) (*sql.DB, error) {
	...
	if dsn == "" {
		...
		port := getenvDefault("SCS_DB_PORT", "5433")
